...
def list_holdings(month: Optional[str] = Query(None), current_user: str = Depends(require_auth)):
    holdings = get_holdings(current_user)
    if month:
...
def create_holding(body: HoldingCreate, current_user: str = Depends(require_auth)):
    asset_ids = {a["asset_id"] for a in get_assets(current_user)}
    ...
    if any(h["asset_id"] == body.asset_id and h["month_year"] == body.month_year for h in holdings):
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Holding for this asset+month already exists. Edit it from the dashboard.")
    record = {"id": _next_id(holdings), **body.model_dump()}
    holdings.append(record)
    save_holdings(current_user, holdings)
    return HoldingOut(**record)


@router.put("/{holding_id}", response_model=HoldingOut)
def update_holding(holding_id: int, body: HoldingUpdate, current_user: str = Depends(require_auth)):
    holdings = get_holdings(current_user)
    for h in holdings:
        if h["id"] == holding_id:
...
def delete_holding(holding_id: int, current_user: str = Depends(require_auth)):
    holdings = get_holdings(current_user)
    updated = [h for h in holdings if h["id"] != holding_id]
    if len(updated) == len(holdings):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Holding not found.")
    save_holdings(current_user, updated)
